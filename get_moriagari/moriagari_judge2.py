...
def make_histgram(time_list, cnt_list, con_detection_time, con_detection, detection_time, detection):
    plt.plot(time_list, cnt_list)
...
def moriagari_detection(cnt_list,min_list):
    ...
    for i in range(len(cnt_list)):
        ...
        if i == 0 :
            ...
            #　３分間の平均, 標準偏差を計算
            data = np.array([cnt_list[i-1],cnt_list[i-2],cnt_list[i-3]])
            std = np.std(data)
            ave = np.average(data)
        ...
        if cnt_list[i] > moriagari_value[i]:
            ...
            detection_time.append(time_list[i])
            detection.append(175)
        else:
        ...
        try:
            ...
            if moriagari_switch[i-1] == 1 or moriagari_con[i-1] == 1:
                if cnt_list[i] > np.average(data):
                    ...
                    con_detection_time.append(time_list[i])
                    con_detection.append(150)
                else:
    ...
    return con_detection_time, con_detection, detection_time, detection
...
if __name__ == '__main__':
    ...
    con_detection_time, con_detection, detection_time, detection = moriagari_detection(cnt_list, time_list)
    #グラフを作成
    make_histgram(time_list, cnt_list, con_detection_time, con_detection, detection_time, detection)
    #盛り上がり区間の時間幅を抽出
