...
def csv_file_open():
    file = open('0612soccerNico.csv', encoding='utf-8')
...
if __name__ == '__main__':
    ...
    for j in range(len(comment_data)):
        timedata.append(comment_data[j-1][1])
    # for i in range(len(comment_data)):
    #     print(timedata[i-1])
    #
    with open('Nicodata.csv', 'w') as f:    #newline=''を追加した
        writer = csv.writer(f)
        ...
        for k in range(len(comment_data)):
            writer.writerow([comment[k-1],timedata[k-1],PNdata[k-1]])
